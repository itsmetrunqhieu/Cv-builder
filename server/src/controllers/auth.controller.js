...
const forgotpassword = async (req, res, next) => {
  try{
    ...
    const validUser = await User.findOne({ where: { email } });
    if (!validUser) return next(errorHandler(404, "No user created with this email!"));
    const token = ({ id: validUser.id }, process.env.JWT_SECRET, { expiresIn: '15m'});
    ...
    res.status(200, "A reset password link has been sent to your email to reset your password");
  } catch (err) {
    next (err);
  }
}
...
const resetpassword = async (req, res, next) => {
  try{
    ...
    const update = await User.update({ password: hashPassword },{ where: {id:id}});
    if (!update) return next(errorHandler(500, "update failed, please try again later."));
    res
      .status(200, "You have successfully change your password");
  } catch (err){
    next (err);
  }
};
